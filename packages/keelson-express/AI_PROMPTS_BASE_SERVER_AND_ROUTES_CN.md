...
本文档提供了用于AI辅助编程的详细提示词，帮助开发者理解和使用 `@ticatec/keelson-express` 框架中的 BaseServer 和 CommonRoutes 类。
...
4. **DAO 数据访问层 (`dao`)**:
   - 负责最底层的数据库通信与 SQL / ORM 执行（如基于 `@ticatec/keelson-pg` 或数据库连接池执行具体增删改查）。
...
请帮我创建一个完整的 Express 应用程序，使用 @ticatec/keelson-express 框架。要求如下：
...
这些 prompts 可以直接复制给 AI 助手（如 Claude、ChatGPT 等），让它们帮你生成符合 `@ticatec/keelson-express` 框架规范的代码。
