...
Initial release. Extracted from `@ticatec/logger-wrapper` so that Keelson packages depend on a logging *contract* rather than on pino.
...
- **`setLoggerProvider(provider)`** — installs the process-wide logging implementation. Any library can be adapted in a few lines; `@ticatec/logger-wrapper` ships a pino adapter.
